            ...
            if st.button("🔍 Search with AI", key=f"ai_{cid}", use_container_width=True):
                with st.spinner("🤖 Searching..."):
                    try:
                        ...
                        results_found = ai_search_for_evidence(
                        ...
                        if cid in results_found:
                            ...
                            for item in results_found[cid]:
                                url = item['url']
                                if not any(r['url'] == url for r in st.session_state.research_results[cid]):
                                    st.session_state.research_results[cid].append(item)
                                    st.session_state.research_approvals[cid][url] = True
                            
                            st.success(f"✅ Found {len(results_found[cid])} sources!")
                            st.rerun()
                        else:
                        ...
                        st.info("💡 Try the ChatGPT Helper instead")
            ...
            with st.spinner("Regenerating..."):
                try:
                    ...
                    if cid in new_results:
                        ...
                        new_items = [r for r in new_results[cid] if r['url'] not in kept_urls]

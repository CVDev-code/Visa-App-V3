...
with main_tab1:
    ...
    st.markdown("Search for evidence and approve the best sources, then convert to PDFs.")
    ...
    if search_btn:
        with st.spinner("🔎 Searching all criteria... This may take 1-2 minutes."):
            try:
                ...
                st.info("💡 Tip: Check artist name spelling.")
    ...
    # CRITERION SELECTION (Dropdown instead of tabs)
    st.subheader("2️⃣ Review Results by Criterion")
    
    criterion_options = [(cid, f"({cid}) {CRITERIA[cid]}") for cid in CRITERIA.keys()]
    selected_idx = st.selectbox(
        "Select criterion to review:",
        range(len(criterion_options)),
        format_func=lambda i: criterion_options[i][1],
        key="research_criterion_selector"
    )
    
    cid, criterion_label = criterion_options[selected_idx]
    criterion_desc = CRITERIA[cid]
    
    st.markdown(f"### {criterion_label}")
    st.divider()
    
    if cid not in st.session_state.research_results:
        st.info("No results found for this criterion.")
    else:
        items = st.session_state.research_results[cid]
        
        with st.expander(f"📋 {len(items)} sources found - Click to review", expanded=True):
            if not items:
                    ...
                    try:
                        ...
                        if not approved_items:
                            ...
                            if total_converted > 0:
                                st.success(f"✅ Converted {total_converted} sources for Criterion ({cid})! Switch to PDF Highlighter tab.")
                            else:
                                st.warning("⚠️ No PDFs were converted. Check error messages above.")
...
with main_tab2:
    ...
    st.caption("Analyze PDFs and highlight evidence for each criterion")
    ...
    highlighter_options = [(cid, f"({cid}) {CRITERIA[cid]}") for cid in CRITERIA.keys()]
    h_selected_idx = st.selectbox(
        "Select criterion to analyze:",
        range(len(highlighter_options)),
        format_func=lambda i: highlighter_options[i][1],
        key="highlighter_criterion_selector"
    )
    
    cid, h_criterion_label = highlighter_options[h_selected_idx]
    criterion_desc = CRITERIA[cid]
    
    st.markdown(f"### {h_criterion_label}")
    st.divider()
